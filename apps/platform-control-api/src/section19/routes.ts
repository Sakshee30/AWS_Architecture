...
export async function registerSection19Routes(app:FastifyInstance,service:ControlPlaneService,actor:(r:FastifyRequest)=>ControlActor,environment:string){
 ...
 app.get<{Params:{id:string}}>('/platform/changes/:id/status',async(req,reply)=>reply.code(501).send({error:{code:'REPOSITORY_STATUS_ROUTE_REQUIRES_ADAPTER',message:'Wire repository getChange through the composition root',requestId:req.id}}));
