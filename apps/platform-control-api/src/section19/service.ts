...
export class ControlPlaneService {
 ...
 getChange(id:string){return this.repo.getChange(id)}
 async requireChange(id:string){const c=await this.repo.getChange(id);if(!c)throw Object.assign(new Error('Change not found'),{statusCode:404,code:'CHANGE_NOT_FOUND'});return c}
 ...
 async activateConfig(environment:string,version:number,actor:string,correlationId:string){await this.repo.activateConfigVersion(environment,version);await this.audit(actor,'config.activate',String(version),environment,correlationId,'SUCCESS');return{environment,version,active:true}}
