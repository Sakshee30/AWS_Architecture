...
export class PostgresDatabase {
  ...
  async query<T extends QueryResultRow>(sql:string,values:unknown[]=[]):Promise<T[]>{const start=performance.now();const result=await this.pool.query<T>({text:sql,values});const elapsed=performance.now()-start;if(elapsed>=this.slowMs)console.warn(JSON.stringify({event:'slow_query',elapsedMs:Math.round(elapsed),rowCount:result.rowCount,queryName:'unnamed'}));return result.rows;}
